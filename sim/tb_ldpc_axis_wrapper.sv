...
module tb_ldpc_axis_wrapper;
    ...
    task automatic collect_error_output(input string name, input logic expect_early, input logic expect_missing);
        begin
            word_idx = 0;
            wait_guard = 0;
            ...
            if (!frame_error ||
                ...
                missing_tlast_error !== expect_missing) begin
                $display("FAIL %s error flags frame=%0d early=%0d missing=%0d",
                    name, frame_error, early_tlast_error, missing_tlast_error);
                failures = failures + 1;
            end else begin
    ...
    initial begin
        ...
        collect_error_output("early_tlast", 1'b1, 1'b0);
